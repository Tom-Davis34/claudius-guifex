...
1. **AUTHOR** — `writing-component-specs` produces `<Component>.spec.md` (a States table with
   ...
   stylesheet, no JS, no component/module-CSS imports), fluid CSS satisfying the spec's `## Responsive` invariants.
...
5. **GATE 2 (fidelity review)** — depends on the `renderer` config key. `fidelity-storybook`
   ...
   MCP endpoint needed. Either way, the subagent surfaces a per-state `structure | visual | responsive` table (compared at widths 320/768/1280), then **stops for human sign-off**.
